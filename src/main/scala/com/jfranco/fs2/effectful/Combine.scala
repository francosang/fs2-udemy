...
object Combine extends IOApp.Simple {
  override def run: IO[Unit] = {
    ...
    val s5 = Stream.exec(IO.println("Start")) ++ Stream(1, 2, 3) ++ Stream(
      4,
      5,
      6
    ) ++ Stream.exec(IO.println("Finish"))
    s5.compile.toList.flatMap(IO.println)

    val delayed =
      Stream.sleep_[IO](1.second) ++ Stream.eval(IO.println("I am awake!"))
    delayed.compile.drain

    // Exercise
    def evalEvery[A](d: FiniteDuration)(fa: IO[A]): Stream[IO, A] = {
      (Stream.sleep_[IO](1.second) ++ Stream.eval(fa)).repeat
    }

    evalEvery(2.seconds)(IO.println("Hi").as(42))
      .take(5)
      .compile
      .toList
      .flatMap(IO.println)
