...
object Create extends IOApp.Simple {
  override def run: IO[Unit] = {
    val s: Stream[IO, Unit] =
      Stream.eval(IO.println("my first effectful stream!"))
    s.compile.toList.flatMap(IO.println)
    s.compile.drain

    val s2: Stream[IO, Nothing] =
      Stream.exec(IO.println("my second effectful stream!"))
    s2.compile.drain
    ...
    val natsEval =
      Stream.iterateEval(1)(a => IO.println(s"Producing ${a + 1}") *> IO(a + 1))
