...
object ParEvalMap extends IOApp.Simple {
  override def run: IO[Unit] = {
    ...
    val jobs: Stream[IO, Job] =
      Stream.unfold(1)(id => Some((Job(id, Created), id + 1))).covary[IO]

    jobs
      .parEvalMapUnordered(5)(
        processJob
      ) // process in parallel without restriction, no order kept
      //.parEvalMap(5)(processJob) // process with a max of parallel items
      //.parEvalMapUnbounded(processJob) // process in parallel without restriction keeping order
      //.evalMap(processJob) // process individually keeping order
