...
object Merge extends IOApp.Simple {
  override def run: IO[Unit] = {
    val s1Inf: Stream[IO, String] =
      Stream.iterate("0")(_ + "1").covary[IO].metered(100.millis)
    val s2Inf: Stream[IO, String] =
      Stream.iterate("z")(_ + "z").covary[IO].metered(200.millis)
    val s3Inf: Stream[IO, String] = s1Inf.merge(s2Inf)
    s3Inf
      .interruptAfter(5.seconds)
      .evalTap(el => IO(println(s"Element: $el")))
      .compile
      .drain

    val s1Failing =
      Stream("a", "b", "c").covary[IO].metered(100.millis) ++ Stream
        .raiseError[IO](new Exception("s1 failed"))
    val s3LeftFailing = s1Failing.merge(s2Inf)
    s3LeftFailing.interruptAfter(5.seconds).printlns.compile.drain

    val s2Failing =
      Stream("a", "b", "c").covary[IO].metered(100.millis) ++ Stream
        .raiseError[IO](new Exception("s2 failed"))
    ...
    //s1Finite.mergeHaltL(s2Finite)
    //s1Finite.mergeHaltR(s2Finite)
    ...
    val source1 = Stream.repeatEval(fetchRandomQuoteFromSource1).take(100)

    Stream
      .repeatEval(fetchRandomQuoteFromSource1)
      .take(100)
      .merge(
        Stream.repeatEval(fetchRandomQuoteFromSource2).take(150)
      )
      .interruptAfter(5.seconds)
      .printlns
      .compile
