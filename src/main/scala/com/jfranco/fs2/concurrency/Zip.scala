...
object Zip extends IOApp.Simple {
  override def run: IO[Unit] = {
    ...
    val s3 = Stream
      .repeatEval(IO(LocalDateTime.now))
      .evalTap(IO.println)
      .metered(1.second)
    s3.zipRight(s2Inf)
    ...
    val s4 =
      Stream.iterateEval(0)(i => IO.println(s"Pulling left $i").as(i + 1))
    val s5 =
      Stream.iterateEval(0)(i => IO.println(s"Pulling right $i").as(i + 1))

    s4.zip(s5) // Runs left and right, left and right, and so on
      .take(15)
      .compile
      .toList
      .flatMap(IO.println)

    s4.parZip(s5) // Runs each one in parallel, keeping tuple order
      .take(15)
      .compile
      .toList
      .flatMap(
