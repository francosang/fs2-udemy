...
object Concurrently extends IOApp.Simple {
  override def run: IO[Unit] = {
    ...
    val s1Failing =
      Stream.repeatEval(IO(42)).take(500).printlns ++ Stream.raiseError[IO](
        new Exception("s1 failed")
      )
    val s2Failing =
      Stream.repeatEval(IO(6000)).take(500).printlns ++ Stream.raiseError[IO](
        new Exception("s2 failed")
      )
    ...
    def progressTracker(itemsProcessed: Ref[IO, Int]) =
      Stream
        .repeatEval(
          itemsProcessed.get.flatMap(n =>
            IO.println(s"Progress: ${n * 100 / numItems} %")
          )
        )
        .metered(100.millis)
        .drain
