...
object TimedRevisited extends IOApp.Simple {
  lazy val format = DateTimeFormatter.ofPattern("hh:mm:ss")
  ...
  override def run: IO[Unit] = {
    ...
    val awakeEveryStream: Stream[IO, FiniteDuration] =
      Stream.awakeEvery[IO](2.seconds)
    ...
    val awakeDelayStream: Stream[IO, FiniteDuration] =
      Stream.awakeDelay[IO](2.seconds)
